...
- [ ] **Step 1: Verify call sites are passing `_effective` values**
  ...
  - effTS = eff?.thunderstorm_probability ?? null ✓  (_effective.thunderstormProb)
  - effHI = eff?.heat_index ?? null ✓  (_effective.heatIndex)
  - effPP = eff?.precipitation_probability ?? null ✓  (_effective.precipProb)
...
- [ ] **Step 3: Add 3-stop vertical density gradient to fog render**
  ...
  **Step 3b: Per-blob warm/cold colour interpolation (fog colour temperature)**
  ...
  // state._temperature = this._temperature, set from effTemp in start()/updateDynamic()
  // i.e. this IS _effective.temperature — no separate lookup needed
