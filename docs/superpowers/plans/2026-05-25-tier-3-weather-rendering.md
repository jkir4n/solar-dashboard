...
- [ ] **Step 5: Verify (visual)**
  ...
  - Fog colour should appear slightly warmer (warm tint from `sunElevation`). (Note: warm tint is applied by existing `_wft` wind-feel tint system via the condition colour; the spec notes this is the existing system. No additional code change needed for the tint itself.)
...
- [ ] **Step 2: Replace rainbow render block with afterglow + secondary bow**
  ...
  const secondaryAlpha = baseAlpha * 0.15 * cloudDimRainbow;
...
`phaseAngle` computed by `computeMoonPhaseAngle()` is in degrees (0–360). `Math.cos()` takes radians — the formula `(1 + Math.cos(phaseAngle * Math.PI / 180)) / 2` correctly multiplies by `Math.PI / 180` before the cosine call.
